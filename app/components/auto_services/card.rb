  ...
  def view_template
    div(class: [ "space-y-0.5", (@service.fresh_for_passenger? ? "animate-shimmer-bottom" : nil) ]) do
      ...
      if authenticated? && @entry.user_id == Current.user.id && request.params[:mode] == "services"
        div(class: "flex flex-wrap gap-2 pt-1") do
          a(href: edit_auto_service_path(@entry), class: "btn btn-xs") { "Изменить" }
          a(
            href: auto_service_path(@entry),
            class: "btn btn-xs btn-outline",
            data: { turbo_method: :delete, turbo_confirm: "Удалить услугу в корзину?" }
          ) { "Удалить" }

          a(
            href: set_activity_auto_service_path(@entry, activity_state: AutoService::STATE_MANUAL),
            class: "btn btn-xs #{@service.manual? ? 'btn-success' : 'btn-ghost'}",
            data: { turbo_method: :patch }
          ) { "Активен" }

          a(
            href: set_activity_auto_service_path(@entry, activity_state: AutoService::STATE_SCHEDULE),
            class: "btn btn-xs #{@service.schedule? ? 'btn-success' : 'btn-ghost'}",
            data: { turbo_method: :patch }
          ) { "Активность по графику" }

          a(
            href: set_activity_auto_service_path(@entry, activity_state: AutoService::STATE_OFF),
