...
class Components::Entries::Card < Phlex::HTML
  ...
  def initialize(entry:, highlight: false, is_last_in_group: true)
    @entry = entry
    @highlight = highlight
  ...
  def view_template
    ...
    cache([ :card, @entry, @entry.updated_at, @avatar, @username, @highlight ]) do
      div(id: dom_id(@entry),
        ...
        div(class: [ "chat-bubble max-w-[99%]", ("before:hidden" unless @is_last_in_group), (@highlight ? "animate-shimmer-bottom" : nil) ]) do
          case @entry.entryable
          when Post
            render Components::Posts::Card.new(post: @entry.entryable) do |card|
              card.content
