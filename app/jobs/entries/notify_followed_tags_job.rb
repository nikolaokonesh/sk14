class Entries::NotifyFollowedTagsJob < ApplicationJob
  ...
  def perform(entry_id)
    ...
    return if entry.tag_ids.empty?
    return if Noticed::Event.exists?(type: "Entries::NewEntryInFollowedTagsNotifier", record: entry)

    recipients = User.joins(:subscriptions)
                     .where(subscriptions: { followable_type: "Tag", followable_id: entry.tag_ids })
                     .where.not(id: entry.user_id).distinct
