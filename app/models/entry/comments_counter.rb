module Entry::CommentsCounter
  ...
  def decrement_root_comments_count
    ...
    Entry.where(id: root_id).update_all("comments_count = GREATEST(COALESCE(comments_count, 0) - 1, 0)")
  end

  def move_root_comments_count
    previous_root_id, new_root_id = saved_change_to_root_id

    Entry.where(id: previous_root_id).update_all("comments_count = GREATEST(COALESCE(comments_count, 0) - 1, 0)") if previous_root_id
